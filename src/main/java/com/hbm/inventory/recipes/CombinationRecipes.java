...
public class CombinationRecipes {
    ...
    public static Tuple.Pair<ItemStack, FluidStackHBM> getOutput(ItemStack stack) {
        ...
        // 2. Проверка по TagKey (НОВОЕ!)
        for (Map.Entry<Object, Tuple.Pair<ItemStack, FluidStackHBM>> entry : recipes.entrySet()) {
            Object key = entry.getKey();
            if (key instanceof TagKey) {
                // Пытаемся получить TagKey<Item>
                @SuppressWarnings("unchecked")
                TagKey<Item> itemTag = (TagKey<Item>) key;
                if (stack.is(itemTag)) {
                    Tuple.Pair<ItemStack, FluidStackHBM> out = entry.getValue();
                    return new Tuple.Pair<>(
                            out.key() == null ? null : out.key().copy(),
                            out.value() == null ? null : out.value().copy()
                    );
        ...
        // 3. Проверка по строковым ключам (старая логика)
        ...
        // 4. Проверка по путям тегов без namespace
