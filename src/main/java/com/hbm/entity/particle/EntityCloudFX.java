...
public class EntityCloudFX extends EntityModFX {
    ...
    public EntityCloudFX(EntityType<?> type, Level level) {
        super(type, level);
    }

    public EntityCloudFX(EntityType<?> type, Level level, double x, double y, double z, double mx, double my, double mz) {
    ...
    public EntityCloudFX(Level level, double x, double y, double z, double mx, double my, double mz) {
        this(ModEntities.CLOUD_FX.get(), level, x, y, z, mx, my, mz);
    }

    public EntityCloudFX(EntityType<?> type, Level level, double x, double y, double z, double mx, double my, double mz, float scale) {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide && random.nextInt(50) == 0) {
            ...
            ExplosionChaos.cloud(this.level(), pos.getX(), pos.getY(), pos.getZ(), 2);
        }
