...
public abstract class GlyphidStats {
    ...
    public static class StatBundle {
        ...
        public StatBundle(double health, double speed, double damage, float divisor, float damageThreshold,
                          float thresholdMultPerArmor, float resistanceMult) {
            this.health = health;
            this.speed = speed;
            this.damage = damage;
    ...
    public static class GlyphidStats70K extends GlyphidStats {
        ...
        public boolean handleAttack(EntityGlyphid glyphid, DamageSource source, float amount) {
            // В 1.20.1 используем теги для проверки типов урона
            if (!source.is(DamageTypeTags.BYPASSES_ARMOR) &&
                    !source.is(DamageTypeTags.BYPASSES_INVULNERABILITY) &&
                    !glyphid.level().isClientSide &&
                    !source.is(DamageTypeTags.IS_FIRE) &&
                    !source.getMsgId().equals(ModDamageSource.s_cryolator)) {

                byte armor = glyphid.getEntityData().get(glyphid.DW_ARMOR);
            ...
            if (source.is(DamageTypeTags.IS_FIRE)) {
                ...
                double scale = glyphid.getScale();
                amount *= scale < 1.25 ? 1.5 : scale < 1.3 ? 0.8 : 0.5;
            } else if (source.is(ModDamageSource.ACID) || ModDamageSource.s_acid.equals(source.getMsgId())) {
                amount = 0;
            } else if (source.is(DamageTypeTags.IS_EXPLOSION)) {
                // InWall в 1.20.1 нет прямого аналога, используем проверку на удушение
            ...
            // Проверка эффектов
            MobEffectInstance effect = glyphid.getEffect(HbmPotion.PHOSPHORUS.get());
            if (effect != null) {
                amount *= 1.5F;
            }
    ...
    public static class GlyphidStatsNT extends GlyphidStats {
        ...
        public boolean handleAttack(EntityGlyphid glyphid, DamageSource source, float amount) {
            ...
            if ((source.is(ModDamageSource.ACID) || ModDamageSource.s_acid.equals(source.getMsgId())) &&
                    source.getDirectEntity() instanceof EntityGlyphid) {
                return false;
            }
            return glyphid.attackSuperclass(source, amount);
        }
