...
public class TileEntityLaunchTable extends TileEntityMachineBase implements
    ...
    private final ItemStackHandler inventory = new ItemStackHandler(8) {
        ...
        public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
            return switch (slot) {
                ...
                case 4 -> stack.getItem() == ModItems.ROCKET_FUEL.get();
                case 2, 3, 5, 6, 7 -> true;
                default -> false;
            };
        }
    ...
    public TileEntityLaunchTable(BlockPos pos, BlockState state) {
        super(ModTileEntity.LAUNCH_TABLE.get(), pos, state);
        this.tanks = new FluidTankHBM[2];
        this.tanks[0] = new FluidTankHBM(Fluids.NONE.get(), 100_000);
        this.tanks[1] = new FluidTankHBM(Fluids.NONE.get(), 100_000);
    }
    ...
    private void updateConnections() {
        // Большая площадка: подключения по периметру 9x9
        for (int i = -4; i <= 4; i++) {
            trySubscribe(level, worldPosition.offset(i, 0, 5), Direction.SOUTH);
            trySubscribe(level, worldPosition.offset(i, 0, -5), Direction.NORTH);
            trySubscribe(level, worldPosition.offset(5, 0, i), Direction.EAST);
            trySubscribe(level, worldPosition.offset(-5, 0, i), Direction.WEST);

            for (int j = 0; j < 2; j++) {
                trySubscribe(tanks[j].getTankType(), level, worldPosition.offset(i, 0, 5), Direction.SOUTH);
                trySubscribe(tanks[j].getTankType(), level, worldPosition.offset(i, 0, -5), Direction.NORTH);
                trySubscribe(tanks[j].getTankType(), level, worldPosition.offset(5, 0, i), Direction.EAST);
                trySubscribe(tanks[j].getTankType(), level, worldPosition.offset(-5, 0, i), Direction.WEST);
            }
        }
    }
    ...
    public MissileStruct getStruct() {
        return TileEntityLaunchTable.getStruct(inventory.getStackInSlot(0));
    }
    ...
    public AbstractContainerMenu createMenu(int id, @NotNull Inventory inv, @NotNull Player player) {
        return new ContainerLaunchTable(id, inv, this);
    }

    @Override
    public net.minecraft.network.chat.@NotNull Component getDisplayName() {
        return net.minecraft.network.chat.Component.translatable("container.hbm.launch_table");
    }
    ...
    public void saveAdditional(@NotNull CompoundTag nbt) {
        ...
        nbt.putInt("padSize", padSize.ordinal());
    ...
    public void load(@NotNull CompoundTag nbt) {
        ...
        padSize = PartSize.values()[nbt.getInt("padSize")];
    ...
    public void setPadSize(PartSize size) {
        this.padSize = size;
        setChanged();
    }
}