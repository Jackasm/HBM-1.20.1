...
public class RenderMissileNuclear extends EntityRenderer<Entity> {
    ...
    public void render(@NotNull Entity entity, float entityYaw, float partialTicks,
        ...
        // === 1. Выбор текстуры ===
        ResourceLocation texture = HBMResourceManager.missileNuclear_tex;
        if (entity instanceof EntityMissileNuclear) texture = HBMResourceManager.missileNuclear_tex;
        else if (entity instanceof EntityMissileMirv) texture = HBMResourceManager.missileThermo_tex;
        else if (entity instanceof EntityMissileDoomsday) texture = HBMResourceManager.missileDoomsday_tex;
        else if (entity instanceof EntityMissileDoomsdayRusted) texture = HBMResourceManager.missileDoomsdayRusted_tex;
        else if (entity instanceof EntityMissileVolcano) texture = HBMResourceManager.missileVolcano_tex;
        ...
        // === 4. Масштаб ===
        // Nuclear ракеты крупнее
        float scale = 2.5F;
        poseStack.scale(scale, scale, scale);

        // === 5. Рендерим модель ===
        VertexConsumer consumer = buffer.getBuffer(RenderType.entityCutout(texture));
        HBMResourceManager.missileNuclear.renderAll(poseStack, consumer, packedLight, OverlayTexture.NO_OVERLAY);
    ...
    public @NotNull ResourceLocation getTextureLocation(@NotNull Entity entity) {
        return HBMResourceManager.missileNuclear_tex;
    }
}