...
public class RenderMissileHuge extends EntityRenderer<Entity> {
    ...
    public void render(@NotNull Entity entity, float entityYaw, float partialTicks,
        ...
        // === 1. Выбор текстуры ===
        ResourceLocation texture = HBMResourceManager.missileHuge_HE_tex;
        if (entity instanceof EntityMissileBurst) texture = HBMResourceManager.missileHuge_HE_tex;
        else if (entity instanceof EntityMissileInferno) texture = HBMResourceManager.missileHuge_IN_tex;
        else if (entity instanceof EntityMissileRain) texture = HBMResourceManager.missileHuge_CL_tex;
        else if (entity instanceof EntityMissileDrill) texture = HBMResourceManager.missileHuge_BU_tex;
        ...
        // === 4. Масштаб ===
        // Для Huge ракет можно увеличить масштаб
        float scale = 2.0F;
        poseStack.scale(scale, scale, scale);

        // === 5. Рендерим модель ===
        VertexConsumer consumer = buffer.getBuffer(RenderType.entityCutout(texture));
        HBMResourceManager.missileHuge.renderAll(poseStack, consumer, packedLight, OverlayTexture.NO_OVERLAY);
    ...
    public @NotNull ResourceLocation getTextureLocation(@NotNull Entity entity) {
        return HBMResourceManager.missileHuge_HE_tex;
    }
}