...
@OnlyIn(Dist.CLIENT)
public class ModelArmorFAU extends ModelArmorBase {

    public static final ModelLayerLocation[] DIGAMMA_LAYERS = new ModelLayerLocation[]{
            new ModelLayerLocation(ResLocation(RefStrings.MODID, "digamma_helmet"), "main"),
            new ModelLayerLocation(ResLocation(RefStrings.MODID, "digamma_chest"), "main"),
            new ModelLayerLocation(ResLocation(RefStrings.MODID, "digamma_legs"), "main"),
            new ModelLayerLocation(ResLocation(RefStrings.MODID, "digamma_boots"), "main")
    };

    private ModelRendererObj cassette;

    public ModelArmorFAU(ModelPart root, int type) {
        super(type, root);
    ...
    public void renderToBuffer(@NotNull PoseStack poseStack, @NotNull VertexConsumer vertexConsumer,
        ...
        if (this.type == 3) {
            VertexConsumer consumer = Minecraft.getInstance().renderBuffers().bufferSource()
                    .getBuffer(RenderType.entityCutoutNoCull(ResLocation(RefStrings.MODID, "textures/models/armor/fau_helmet.png")));
            this.head.render(poseStack, consumer, packedLight, packedOverlay, scale);
        }
        if (this.type == 2) {
            VertexConsumer consumer = Minecraft.getInstance().renderBuffers().bufferSource()
                    .getBuffer(RenderType.entityCutoutNoCull(ResLocation(RefStrings.MODID, "textures/models/armor/fau_chest.png")));
            this.body.render(poseStack, consumer, packedLight, packedOverlay, scale);
            this.cassette.render(poseStack, consumer, packedLight, packedOverlay, scale);
            consumer = Minecraft.getInstance().renderBuffers().bufferSource()
                    .getBuffer(RenderType.entityCutoutNoCull(ResLocation(RefStrings.MODID, "textures/models/armor/fau_arm.png")));
        ...
        if (this.type == 1) {
            VertexConsumer consumer = Minecraft.getInstance().renderBuffers().bufferSource()
                    .getBuffer(RenderType.entityCutoutNoCull(ResLocation(RefStrings.MODID, "textures/models/armor/fau_leg.png")));
