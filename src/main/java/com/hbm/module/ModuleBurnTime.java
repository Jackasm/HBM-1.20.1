...
public class ModuleBurnTime {
    ...
    public double getMod(ItemStack stack, double[] mod) {
        ...
        if (stack.getItem() == ModItems.SOLID_FUEL.get()) return mod[modSolid];
        if (stack.getItem() == ModItems.SOLID_FUEL_PRESTO.get()) return mod[modSolid];
        if (stack.getItem() == ModItems.SOLID_FUEL_PRESTO_TRIPLET.get()) return mod[modSolid];

        if (stack.getItem() == ModItems.SOLID_FUEL_BF.get()) return mod[modBalefire];
        if (stack.getItem() == ModItems.SOLID_FUEL_PRESTO_BF.get()) return mod[modBalefire];
        if (stack.getItem() == ModItems.SOLID_FUEL_PRESTO_TRIPLET_BF.get()) return mod[modBalefire];

        if (stack.getItem() == ModItems.ROCKET_FUEL.get()) return mod[modRocket];
    ...
    public List<Component> getTimeDesc() {
        ...
        list.add(Component.literal(ChatFormatting.GOLD + "Burn time bonuses:"));
    ...
    public List<Component> getHeatDesc() {
        ...
        list.add(Component.literal(ChatFormatting.RED + "Burn heat bonuses:"));
    ...
    private void addIf(List<Component> list, String name, double mod) {
        if (mod != 1.0D) {
            list.add(Component.literal(ChatFormatting.YELLOW + "- " + name + ": " + getPercent(mod)));
        }
    }
