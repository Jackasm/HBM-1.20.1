...
import com.hbm.render.item.armor.ArmorAJRItemsRenderer;
...
public class ArmorAJR extends ArmorFSBPowered {
    ...
    @OnlyIn(Dist.CLIENT)
    private ModelArmorAJR[] models;

    public ArmorAJR(ArmorMaterial material, Type type, Properties properties,
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            ...
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity living, ItemStack stack, EquipmentSlot slot, HumanoidModel<?> defaultModel) {
                if (models == null) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        ModelPart root = mc.getEntityModels().bakeLayer(ModelArmorAJR.AJR_LAYERS[i]);
                        models[i] = new ModelArmorAJR(root, i);
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                return new ArmorAJRItemsRenderer();
            }
        });
    }
