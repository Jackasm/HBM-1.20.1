...
public class ItemToolAbilityPower extends ItemToolAbility {
    ...
    public ItemToolAbilityPower(Properties properties, Tier tier, TagKey<Block> effectiveBlocks,
                                float attackDamage, float attackSpeed,
                                long maxPower, long chargeRate, long consumption) {
        super(attackDamage, attackSpeed, tier, effectiveBlocks, properties, null);
    ...
    public ItemToolAbilityPower addToolAreaAbility(IToolAreaAbility ability, int level) {
        this.availableAbilities.addAbility(ability, level);
        return this;
    }

    public ItemToolAbilityPower addToolHarvestAbility(IToolHarvestAbility ability, int level) {
        this.availableAbilities.addAbility(ability, level);
        return this;
    }

    public ItemToolAbilityPower addWeaponAbility(com.hbm.handler.ability.IWeaponAbility ability, int level) {
        this.availableAbilities.addAbility(ability, level);
        return this;
    }

    public ItemToolAbilityPower setShears() {
        this.isShears = true;
        return this;
    }

    public ItemToolAbilityPower setRockBreaker() {
    ...
    public boolean mineBlock(@NotNull ItemStack stack, @NotNull Level level,
        ...
        if (!level.isClientSide && entity instanceof Player && canOperate(stack)) {
            dischargeBattery(stack, consumption);
        }
