...
import com.hbm.create.blockentity.ItemRequirement.ItemUseType;
...
public class FramedBlocksInSchematics {
    ...
    public static ItemRequirement getRequiredItems(BlockState blockState, BlockEntity blockEntity) {
        ...
        if (data.getBoolean("intangible"))
            list.add(new ItemRequirement.StackRequirement(new ItemStack(Items.PHANTOM_MEMBRANE), ItemUseType.CONSUME));

        if (data.getBoolean("glowing"))
            list.add(new ItemRequirement.StackRequirement(new ItemStack(Items.GLOWSTONE_DUST), ItemUseType.CONSUME));

        if (data.getBoolean("reinforced"))
            list.add(new ItemRequirement.StackRequirement(new ItemStack(Mods.FRAMEDBLOCKS.getItem("framed_reinforcement")),
    ...
    private static void addCamoStack(HolderGetter<Block> level, CompoundTag tag, List<ItemRequirement.StackRequirement> list) {
        ...
        if (!itemStack.isEmpty())
            list.add(new ItemRequirement.StackRequirement(itemStack, ItemUseType.CONSUME));
    }
